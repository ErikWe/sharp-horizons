...
/// <inheritdoc cref="IOriginDataInterpreter"/>
internal sealed class OriginDataInterpreter : IOriginDataInterpreter
{
    ...
    private Dictionary<string, Action<string, MutableOriginDataInterpretation>> Interpreters { get; } = new(5);

    /// <inheritdoc cref="OriginDataInterpreter"/>
    ...
    /// <param name="originInterpretationKeyProvider"><inheritdoc cref="OriginInterpretationKeyProvider" path="/summary"/></param>
    ...
    public OriginDataInterpreter(IInterpretationOptionsProvider interpretationOptionsProvider, IEphemerisInterpretationOptionsProvider ephemerisInterpretationOptionsProvider, IOriginInterpretationKeyProvider originInterpretationKeyProvider, IOriginInterpreter originInterpreter,
        IOriginGeodeticCoordinateInterpreter geodeticCoordinateInterpreter, IOriginCylindricalCoordinateInterpreter cylindricalCoordinateInterpreter, IOriginReferenceEllipsoidInterpreter referenceEllipsoidInterpreter, IOriginRadiiInterpreter radiiInterpreter)
    {
        ...
        RegisterInterpreter(originInterpreter, originInterpretationKeyProvider.BodyName, static (origin, interpretation) => interpretation.Origin = origin);
        RegisterInterpreter(geodeticCoordinateInterpreter, originInterpretationKeyProvider.GeodeticCoordinate, static (target, interpretation) => interpretation.GeodeticCoordinate = target);
        RegisterInterpreter(cylindricalCoordinateInterpreter, originInterpretationKeyProvider.CylindricalCoordinate, static (target, interpretation) => interpretation.CylindricalCoordinate = target);
        RegisterInterpreter(referenceEllipsoidInterpreter, originInterpretationKeyProvider.ReferenceEllipsoid, static (referenceEllipsoid, interpretation) => interpretation.ReferenceEllipsoid = referenceEllipsoid);
        RegisterInterpreter(radiiInterpreter, originInterpretationKeyProvider.Radii, static (radii, interpretation) => interpretation.Radii = radii);
    }
    ...
    private void RegisterInterpreter<TInterpretation>(IPartInterpreter<TInterpretation> interpreter, string key, Action<TInterpretation, MutableOriginDataInterpretation> setter)
    {
        Interpreters[FormatKey(key)] = (line, interpretation) => setter(interpreter.Interpret(line), interpretation);
    }

    Optional<IOriginDataInterpretation> IInterpreter<IOriginDataInterpretation>.TryInterpret(IQueryResult queryResult)
    {
        MutableOriginDataInterpretation? interpretatation = new();
        ...
        if (interpretatation.Origin is null)
        {
            return new();
        }
    ...
    /// <summary>A mutable <see cref="IOriginDataInterpretation"/>.</summary>
    private sealed class MutableOriginDataInterpretation : IOriginDataInterpretation
    {
